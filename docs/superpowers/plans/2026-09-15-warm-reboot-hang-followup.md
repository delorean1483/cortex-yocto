...
1. **Capture the hang point first (cheapest, do this before anything else).** With serial attached, trigger a warm `reboot` and record the **last serial line before silence**. That single line says which stage wedges (TF-A vs SPL DDR vs u-boot) and disambiguates the fixes below. We never captured it (serial got unplugged mid-session).
