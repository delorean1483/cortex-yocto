...
- **Optimized RTCC batch commit** — Task 8 commits once per RTCC field write (seed-from-live then commit) to honor M4a's stage-seeding seam; a batched "write 42–48 then one commit" optimization can replace it at UART integration if the per-field I²C cost matters. Same fan-out applies to `mbp_nvm` (one journal append per register in a multi-setting FC-0x10 write) — watch flash-wear/latency at bench.
- **NVM commit-failure surfacing (from M4b final review):** `mbp_nvm.c` `wr_nvm` discards `nvm_commit()`'s return, so a failed flash persist is reported to the Modbus master as a successful write. To fix at bench: add a `SLAVE_DEVICE_FAILURE` (0x04) code to `modbus_exc_t` + `make_exception`, and have `wr_nvm` (and any writer whose commit can fail) return it when `nvm_commit()` != 0. Untestable on host (fake_nor never fails commit) — lands with the real S25FL064 NOR driver.
- **RTCC out-of-order individual writes (bench):** a display that sets the clock via separate FC-0x06 writes in a non-ascending field order could momentarily stage an invalid date (e.g. new month before new date). Ascending order (the normal FC-0x10 case) is safe. Confirm the display's write order at bring-up.
