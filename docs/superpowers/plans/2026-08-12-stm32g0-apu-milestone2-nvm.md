...
- **`drv_s25fl064`** — the SPI2 driver implementing `nvm_backend_t` against the real S25FL064 NOR (JEDEC-ID probe, read `0x03`, page-program `0x02`, sector-erase `0x20`, RDSR busy-poll, WREN). Dedicate a NOR region of **≥ 2 sectors** (recommend ≥ 16 for wear spread) to the parameter ring. Wire it in the same milestone that lands the CubeMX SPI2 config (with Task 1 of Milestone 1). Its correctness is validated on hardware (read/write/erase round-trip); the journal logic above is already proven on the fake.
  - **⚠️ CRITICAL page-boundary requirement (found in M2 final review):** `NVM_RECORD_SIZE = 264` is **not** aligned to the S25FL064's 256-byte program page — records straddle page boundaries (slot 1 = bytes 264..527 spans pages 1–2, etc.). A real page-program that crosses a 256-byte boundary **wraps to the start of the same page and overwrites earlier bytes**, silently corrupting records. The in-memory fake models flat memory and CANNOT catch this. Therefore `drv_s25fl064.program()` **MUST split every write at 256-byte page boundaries** (program up to the next page end, then continue). Add a bring-up acceptance test that programs a record straddling a page boundary and reads it back byte-for-byte. Do not skip this — it is the most likely field-corruption bug for this driver.
  - **Geometry guard:** `nvm_init` now defensively enters RAM-only mode (factory defaults in shadow, no flash access, `commit` a no-op) if the backend reports `sector_count < 2` or a sector smaller than one record — so a misconfigured driver degrades safely instead of dividing by zero. The driver must still report a real ≥2-sector geometry for persistence to work.
